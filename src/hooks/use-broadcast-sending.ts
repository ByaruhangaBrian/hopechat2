...
/**
 * Resolve an AudienceConfig to the matching contact rows. Shared by the
 * WhatsApp and SMS broadcast flows (SMS filters for phone numbers at the
 * call site). Runs against the signed-in user's RLS-scoped client, so
 * contact/tag/custom-value lookups stay inside their own business.
 */
export async function resolveAudience(audience: AudienceConfig): Promise<Contact[]> {
