...
export default function LogsPage() {
  ...
  const getStatusBadge = (code: number) => {
    if (!code) return <Badge variant="outline" className="text-muted-foreground border-border">N/A</Badge>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Monitor all platform activity including auth, messages, and system events.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" className="border-border text-muted-foreground" onClick={fetchLogs} disabled={loading}>
            Refresh
          </Button>
        </div>
      </div>
        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow className="hover:bg-transparent border-border">
              ...
              <TableHead className="text-muted-foreground">Tenant/User</TableHead>
                ...
                <TableRow key={log.id} className="border-border hover:bg-muted/30 group">
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[10px] font-mono text-muted-foreground/60">B: {log.business_id?.slice(0, 6) || "SYS"}</span>
                      {log.user_id && <span className="text-[10px] font-mono text-muted-foreground/40">U: {log.user_id.slice(0, 6)}</span>}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      {getServiceIcon(log.service)}
                      <span className="text-xs uppercase font-bold text-muted-foreground/80">{log.service}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-foreground">{log.note?.replace(/_/g, ' ') || log.endpoint}</span>
                      <span className="text-[10px] text-muted-foreground/60 font-mono truncate max-w-[150px]">{log.endpoint}</span>
                    </div>
                  </TableCell>
