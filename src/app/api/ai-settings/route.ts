...
export async function GET() {
  ...
  return NextResponse.json({
    settings: {
      ...
      system_prompt: data?.system_prompt ?? 'You are a helpful customer service AI assistant.',
...
export async function POST(req: Request) {
  ...
  const keyToSave = groq_api_key?.trim()
    ? encrypt(groq_api_key.trim())
    : existing?.groq_api_key ?? ''

  if (is_enabled && !keyToSave) {
    return NextResponse.json(
    ...
    .upsert(
      {
        ...
        system_prompt: system_prompt ?? 'You are a helpful customer service AI assistant.',
